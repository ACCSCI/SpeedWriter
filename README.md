...
## v1.5.3 - 角色锁定、生成取消、章节管理与导入重构

角色锁定与生成管理大版本：AI 写手不会再碰你锁定的角色设定；生成过程可随时中止并回滚；章节侧边栏支持新建和删除；导入流程支持拖拽、文件上传和异步进度追踪。

- **角色锁定系统**：可按角色锁定人格、动机、关系和状态，AI 写手、工具层和 Studio API 三层联动拒绝修改。角色侧边栏提供一键全部锁定/解锁、防止新增角色和防止删除角色等全局开关。
- **生成取消与回滚**：生成过程中可随时中止，系统自动恢复到生成前的临时快照，不会残留半成品章节或污染状态。
- **章节管理**：侧边栏支持新建空白章节并直接打开编辑，每章可独立删除并确认，不再需要 CLI 操作。
- **导入重构**：支持拖拽、文件上传和手动添加章节，可拖拽排序、行内重命名、展开预览，导入异步执行并显示实时进度。
- **Truth 文件 CRUD**：Truth Files 页面支持新建和删除文件，运行时诊断文件受保护不可删除。
- **书级设置可编辑**：书名、题材和平台字段可在 Book Settings 面板直接修改。

<p align="center">
  <img src="assets/studio-play-1-5.png" width="900" alt="InkOS Studio 角色锁定与章节管理界面">
</p>
...
`inkos import chapters` 从已有小说文本导入章节，支持拖拽、文件上传（`.txt` / `.md`）和手动添加章节，可拖拽排序、行内重命名和展开预览。导入异步执行，实时显示基础生成、风格提取和逐章分析进度。支持导入时直接创建新书。自动重建结构化状态、章节摘要、伏笔、角色关系和可读 Markdown 投影，导入后 `inkos write next` 可继续创作。
